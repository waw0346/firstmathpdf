---
tags: [solution-draft, additional-solution, needs-review]
problem_id: 2026_수능_고3_007
new_id: H2611-SU-Q007
solution_type: additional_draft
status: needs_official_solution_access
updated: 2026-06-06 11:14
---

# 추가 해설 초안: 2026_수능_고3_007

## 원본 보존

- 원본/공식 해설 파일: `sources/2026/2026 대학수학능력시험 수학 해설지(EBS).pdf`
- 이 노트는 추가 해설 초안이며 원본 해설을 대체하지 않습니다.
- 공식 해설 접근 또는 교사 검수 전에는 `stage3_sol: pass`로 올리지 않습니다.

## 문제 정보

- 제목: 두 곡선과 직선으로 둘러싸인 넓이 (3점)
- 단원: 적분의 활용
- 난이도: 중
- 정답: 4
- 문제 이미지: ![](sources/crops/2026_수능_고3_007.jpg)

## 추가 해설 작성란

> 공식 해설 확인 후, 원본 표현을 보존하면서 수업용 추가 해설만 작성합니다.

1. 접근 전략:
2. 핵심 계산:
3. 정답 확인:
4. 학생 주의점:

## 검수

- [ ] 공식/원본 해설 대조
- [ ] Math Expert 검토
- [ ] Teacher 수업 표현 검토
- [ ] Verification stage3 반영